...
KBOperate.prototype.findUserForEasyui=function(url,datagridId,condition) {
    findUserByPage(url, datagridId, condition,function (data) {
        if (data.hasPower!=undefined && data.hasPower!=null && data.hasPower.length>0)
        {
            if (data.hasPower[0]==false)//浏览
            {
...
KBOperate.prototype.getMeAsync=function(objID,callback){
    var obj=new Object();
    obj.id=objID;
    doDataAsync(this.baseURL+"/getMe",obj,function (data) {
        callback(data.obj);//回调
    });
}
...
KBOperate.prototype.del=function(objID,callback){
    var obj=new Object();
    obj.id=objID;
    doData(this.baseURL+"/del",obj,function (data) {
        callback(data);
    },true);
}
...
KBOperate.prototype.save=function(checkCallback,obj,afterCallback){
    ...
    if (this.isAdd)
    {
        doData(this.baseURL+"/add",obj,function (data) {
            afterCallback(data);//回调
        },true);
    }
    else
    {
        doData(this.baseURL+"/edit",obj,function (data) {
            afterCallback(data);//回调
        },true);
    }
}
...
* 参数callback：操作完成后需要执行的回调函数，系统会返回服务器执行的结果
************************************************************************************/
KBOperate.prototype.doAsync=function(url,param,callback) {
    doDataAsync(this.baseURL + "/" + url, param, function (data) {
        if (callback != undefined)
            callback(data);
    });
}
...
KBOperate.prototype.doAll=function(url,param,callback){
        doDataAll(url,param,function (data) {
            if (callback!=undefined)
                callback(data);
        });
}

KBOperate.prototype.doAjax=function(url,param,callback){
 ...
 * 参数callback：操作完成后需要执行的回调函数，系统会返回服务器执行的结果
 ***********************************************************************************/
KBOperate.prototype.doAllAsync=function(url,param,callback){
    doDataAsync(url,param,function (data) {
        if (callback!=undefined)
            callback(data);
    });
}
/*设置LayEditor文件上传接口*/
KBOperate.prototype.setLayuiEditorUploadFile = function (layedit) {
    layedit.set({
        uploadImage: {
