...
function formatDateTimebox (value){
    ...
    mm=checkTime(mm);
    s=checkTime(s);
    var ans = y+'-'+(m<10?('0'+m):m)+'-'+(d<10?('0'+d):d)+" "+h+":"+mm+":"+s;
    return ans;
}
...
{
    $("input[type=radio][name="+radioName+"][value="+checkedValue+"]").attr("checked","checked");
}
...
function exit() {
    if(confirm("您确定要退出登录吗？")) {
        doData("/User/logout", new Object(), function (result) {
